...
class FermionGASCF(ABC):
    ...
    def _get_tarr(self, *args):
        N = self.N
        tarr = np.zeros((N, N), dtype=object)
        for I in range(N):
            for J in range(N):
                tarr[I, J] = self._get_tt(I, J, *args)
        return tarr
    ...
    def _compute_Hemb(self, I, Lc, *args):
        r"""
        Compute
    ...
    def _compute_lagrangian(self, x, *args):
        r"""
        Compute
        ...
        for I in range(N):
            Hemb = self._compute_Hemb(I, Lc, *args)
            psi = psiarr[I]
    ...
    def _compute_gradient(self, x, *args, expcorr=False):
       ...
       Here, :math:`B^K` is a diagonal matrix with 
        ...
        for K in range(N):
            ...
            HD = self._compute_Hemb(K, Lc, *args) - Ec[K]*np.eye(4**M)
