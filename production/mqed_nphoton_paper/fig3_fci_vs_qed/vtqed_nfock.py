...
def run_vtqedhf(
    ...
    #vtqedmf.diis_space = 40
    #vtqedmf.precond = 7.5e-3
    ...
    photon_occ = None
    if conv:
        photon_occ = vtqedmf.qed.get_boson_occ()

    del vtqedmf
    return return_dm, return_eta_val, return_fopt_val, e_tot, e_boson, photon_occ, conv
...
if __name__ == "__main__":
    ...
    for la in la_vals:
            ...
            with h5py.File(h5_name, "a") as f:
                ...
                if lambda_group in f:
                    ...
                    if method_group in lam_grp:
                        if DEBUG_VT:
                            print(f"{method_group} data exists for lambda = {la:.3f} in file {h5_name}. Deleting...\n")
                            del lam_grp[method_group]
                            #exit()
                        else:
        ...
        while e_diff >= 1.0e-6:
            ...
            # Run VT-QED-HF
            vtqed_dm, vtqed_eta, vtqed_fopt, e_tot, e_boson, p_occ, conv = run_vtqedhf(
                molecule = molecule,
                basis_set = basis,
                lmbda = la,
                nfock = nf_list,
